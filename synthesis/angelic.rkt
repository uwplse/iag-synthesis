...
(define (symbolize runtime grammar derivation)
  (let* ([oracle (λ (type)
    ...
    (define (recurse node)
      (let* ([symbol (ftl-derivation-symbol derivation)]
             [option (ftl-derivation-option derivation)]
             [bindings (ftl-derivation-attributes derivation)]
             [children (ftl-derivation-children derivation)]
             [alternative (hash-ref (hash-ref vocab symbol) option)]
             [labels (ftl-ir-alternative-labels alternative)]
             [typeof (λ (label)
                       (hash-ref labels label))]
             [noninputs (filter (λ (label)
                                  (not (defined bindings label)))
                                labels)]
             [bind* (λ (label bindings)
                      (cons (cons label ((oracle (typeof label))))
                            bindings))]
             [traverse (λ (child)
                         (if (list? child)
                             (map recurse child)
...
(define (constrain grammar derivation) ; constrain angelic values
  (let* ([recurse (curry constrain grammar)]
         ...
         [children (ftl-derivation-children derivation)]
         [production (hash-ref (hash-ref vocab symbol) option)]
    ...
    (hash-for-each definitions ; FIXME: actually verify that inputs, children given and of the correct types
                   (λ (attribute action)
                     (let* ([def-obj (car attribute)]
                            [def-lbl (cdr attribute)]
                            [def-seq (hash-has-key? sequences def-obj)]
                            [lbl-ref (λ (deriv label)
                                       (lookup (ftl-derivation-attributes deriv)
                                               label))]
                            [reduce (ftl-ir-reduction? action)]
                            [refer (λ (child-attributes accumulator dependency)
                                      (let ([dep-obj (ftl-ir-dependency-object dependency)]
                                        ...
                                        (cond
                                          [(and (eq? dep-obj def-obj)
                                                def-seq
                                                (eq? dep-idx 'current)) (lookup child-attributes dep-lbl)]
                                          [(and (eq? dep-obj def-obj)
                                                reduce
                                                (eq? dep-idx 'previous)) accumulator]
                                          [(and (eq? dep-obj 'self)
                                                (eq? dep-idx 'none)) (lookup attributes dep-lbl)]
                                          [(eq? dep-idx 'none) (lbl-ref (lookup children dep-obj) dep-lbl)]
                                          [(eq? dep-idx 'first) (lbl-ref (first (lookup children dep-obj)) dep-lbl)]
                                          [(eq? dep-idx 'last) (lbl-ref (last (lookup children dep-obj)) dep-lbl)])))]
                            [echo-assert (λ (b)
                                           (display "assertion: ")
                                           (displayln b)
                                           (assert b))])
                       (if reduce
                           (let* ([init (ftl-ir-reduction-init-define action)]
                                  ...
                                  [iter (lookup children (ftl-ir-reduction-iter-child action))]
                                  [proc (λ (child accum)
                                          (let* ([child-attributes (ftl-derivation-attributes child)]
                                                 [result (step-eval (vector-map (curry refer child-attributes accum)
                                                                                step-deps))])
                                            (when def-seq
                                              (echo-assert (equal? (lookup child-attributes def-lbl) result))
                                              result)))]
                                  [seed (init-eval (vector-map (curry refer (void) (void)) init-deps))]
                                  [result (foldl proc seed iter)])
                             (unless def-seq
                               (echo-assert (equal? (lbl-ref (lookup children def-obj) def-lbl)
                                                    result))))
                           (let* ([eval (ftl-ir-definition-evaluate action)]
                                  [deps (ftl-ir-definition-dependencies action)]
                                  [iter (listify (if (eq? def-obj 'self)
                                                     derivation
                                                     (lookup children def-obj)))]
                                  [proc (λ (child)
                                          (let* ([child-attributes (ftl-derivation-attributes child)]
                                                 [result (eval (vector-map (curry refer child-attributes (void))
                                                                           deps))])
                                            (echo-assert (equal? (lookup child-attributes def-lbl) result))))])
                             (for-each proc iter))))))
    (for-each (λ (child)
